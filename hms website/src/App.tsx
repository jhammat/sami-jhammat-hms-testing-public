...
import { PatientJourneyWalkthrough } from "./components/patient-journey-walkthrough";
import { DoctorJourneyWalkthrough } from "./components/doctor-journey-walkthrough";
import { CrossPortalFlows } from "./components/cross-portal-flows";
import { ScreenDemoGallery } from "./components/screen-demo-gallery";
import {
  X,
  ZoomIn,
  ZoomOut,
...
export function App() {
  ...
  const portalCredentials = [
    { role: "Platform Admin", email: "platform@wonflow.local", route: "http://localhost:3007/platform", color: "text-indigo-400" },
    { role: "Hospital Admin", email: "admin@wonflow.local", route: "http://localhost:3007/admin", color: "text-blue-400" },
    { role: "Reception", email: "reception@wonflow.local", route: "http://localhost:3007/operations/reception", color: "text-amber-400" },
    { role: "Doctor", email: "doctor@wonflow.local", route: "http://localhost:3007/doctor", color: "text-emerald-400" },
    { role: "Patient", email: "patient@wonflow.local", route: "http://localhost:3007/patient", color: "text-cyan-400" },
    { role: "Laboratory", email: "laboratory@wonflow.local", route: "http://localhost:3007/operations/laboratory", color: "text-purple-400" },
    { role: "Radiology", email: "radiology@wonflow.local", route: "http://localhost:3007/operations/radiology", color: "text-teal-400" },
    { role: "Pharmacy", email: "pharmacy@wonflow.local", route: "http://localhost:3007/operations/pharmacy", color: "text-rose-400" },
    { role: "Billing", email: "billing@wonflow.local", route: "http://localhost:3007/operations/billing", color: "text-orange-400" },
    { role: "Management", email: "management@wonflow.local", route: "http://localhost:3007/management", color: "text-violet-400" },
  ];
      ...
      <footer className="border-t border-slate-800/80 bg-[#040612] py-12 text-white">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-6">
            ...
            >
              <span>Go to System Login (Port 3007)</span>
            ...
            {portalCredentials.map((cred, idx) => (
              ...
              >
                ...
                <div className="mt-2 pt-2 border-t border-slate-800/60 flex items-center justify-between text-[10px]">
                  <span className="font-mono text-slate-400 truncate max-w-[130px]">{cred.route}</span>
                  <a
                    href={cred.route}
                    target="_blank"
                    rel="noreferrer"
