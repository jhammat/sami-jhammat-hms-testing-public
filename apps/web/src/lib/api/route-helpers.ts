...
export async function requirePlatformContext(): Promise<WonFlowPlatformRequestContext> {
  ...
  if (context.scope !== "platform") throw new WonFlowRequestContextError("permission-required", "Platform access is required.");
  return context;
}
export function safeApiError(error: unknown): NextResponse {
  ...
  if (error instanceof WonFlowRequestContextError) return NextResponse.json({ error: error.message }, { status: error.code === "invalid-request-context" ? 401 : 403 });
