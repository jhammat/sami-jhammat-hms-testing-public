...
import type { ServiceBillingOwner, WorkspaceCode } from "@wonflow/database";
...
export class HospitalAdministrationService {
  ...
  async updateRole(rc:WonFlowRequestContext,id:string,input:{name?:string;description?:string;isActive?:boolean}){const c=this.context(rc);requirePermission(c,"organization.roles.manage");return database.$transaction(async tx=>{const entity=await tx.role.update({where:{id,tenantId:c.tenantId},data:{...input,name:input.name?.trim(),description:input.description?.trim()}});await audit(tx,c,"organization.role.updated","role",entity.id);return entity;});}
  ...
  async listServices(rc:WonFlowRequestContext){const c=this.context(rc);requirePermission(c,"organization.services.read");return database.serviceDefinition.findMany({where:{tenantId:c.tenantId,isActive:true},include:{branch:true,handlerMembership:{select:{id:true,displayName:true,primaryWorkspace:true}},doctor:{include:{staffProfile:{include:{membership:true}}}}},orderBy:{name:"asc"}});}
  ...
  async createService(rc:WonFlowRequestContext,input:{branchId?:string;handlerWorkspace?:WorkspaceCode;handlerMembershipId?:string;doctorId?:string;code?:string;name:string;category:string;description?:string;durationMinutes:number;priceMinorUnits?:number;currencyCode?:string;publiclyBookable?:boolean;billingOwner?:ServiceBillingOwner;consultationMode?:"IN_PERSON"|"ONLINE"}){
    const c=this.context(rc);requirePermission(c,"organization.services.manage");
  ...
  private async persistService(c:WonFlowTenantRequestContext,input:{branchId?:string;handlerWorkspace?:WorkspaceCode;handlerMembershipId?:string;doctorId?:string;name:string;category:string;description?:string;durationMinutes:number;priceMinorUnits?:number;currencyCode?:string;publiclyBookable?:boolean;billingOwner:ServiceBillingOwner;consultationMode?:"IN_PERSON"|"ONLINE"},code:string){
    return database.$transaction(async tx=>{const entity=await tx.serviceDefinition.create({data:{tenantId:c.tenantId,branchId:input.branchId??null,handlerMembershipId:input.handlerMembershipId??null,handlerWorkspace:input.handlerMembershipId?null:(input.handlerWorkspace??null),doctorId:input.doctorId??null,billingOwner:input.billingOwner,code,name:input.name.trim(),category:input.category,description:input.description?.trim()||null,durationMinutes:input.durationMinutes,priceMinorUnits:input.priceMinorUnits??null,currencyCode:input.currencyCode??c.currencyCode,publiclyBookable:input.publiclyBookable??false,consultationMode:input.category==="CONSULTATION"?(input.consultationMode??"IN_PERSON"):"IN_PERSON"}});await audit(tx,c,"organization.service.created","service",entity.id);return entity;});
  }
  async updateService(rc:WonFlowRequestContext,id:string,input:{name?:string;description?:string;handlerWorkspace?:WorkspaceCode|null;handlerMembershipId?:string|null;durationMinutes?:number;priceMinorUnits?:number|null;publiclyBookable?:boolean;isActive?:boolean;billingOwner?:ServiceBillingOwner;consultationMode?:"IN_PERSON"|"ONLINE"}){
    const c=this.context(rc);requirePermission(c,"organization.services.manage");
    ...
    return database.$transaction(async tx=>{const entity=await tx.serviceDefinition.update({where:{id,tenantId:c.tenantId},data:{...input,handlerWorkspace:input.handlerMembershipId?null:input.handlerWorkspace}});await audit(tx,c,"organization.service.updated","service",entity.id);return entity;});
  }
