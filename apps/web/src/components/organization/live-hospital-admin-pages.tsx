...
function groupSchedules(schedules: ScheduleRecord[], doctors: DoctorRecord[]) {
  const groups = new Map<string, { key: string; title: string; subtitle: string; rules: ScheduleRecord[] }>();
...
export function LiveHospitalSchedulesPage() {
  ...
  return <div className="space-y-4" id="main-content"><PageIntro icon={CalendarDays} eyebrow="People & locations" title="Rostered hours" description="Set the hours staff and doctors are expected to attend each branch." /><section className="rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4"><p className="text-sm font-bold text-amber-950">These are expected arrival hours, not bookable slots.</p><p className="mt-1 text-xs leading-5 text-amber-800">Reception books against the doctor&rsquo;s own sitting hours. When a doctor records a sitting for a date, it replaces the roster below for that date. The roster applies only when the doctor has not recorded one.</p></section><WonFlowAsyncDataBoundary loadingTitle="Loading rostered hours" loadingDescription="Reading availability rules from the tenant database." onRetry={resource.reload} state={resource}>{({ schedules, doctors, configuration, services }) => <ScheduleManager branches={configuration.branches} doctors={doctors} onCreated={resource.reload} schedules={schedules} services={services} />}</WonFlowAsyncDataBoundary></div>;
}
