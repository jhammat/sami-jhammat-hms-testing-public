...
/** Renders strictly from the server order/result — the same record radiology staff release, never a locally cached copy. */
export function PrintableRadiologyReport({ orderId }: { orderId: string }) {
  ...
  if (!released.length) return <p className="p-6 text-sm text-red-700">No released radiology report exists for this order.</p>;
  ...
  return (
    <div className="space-y-5">
      <div className="wf-no-print flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4">
        <Link className="wf-button-secondary" href={`/operations/radiology/results/${encodeURIComponent(orderId)}`}>Return to Result</Link>
        <WonFlowActionButton icon={<Printer size={17} />} onClick={() => window.print()} variant="primary">Print Radiology Report</WonFlowActionButton>
      </div>
      <article className="wf-print-report mx-auto max-w-[1000px] bg-white p-8 text-slate-950 shadow-sm print:shadow-none">
        <header className="flex items-center gap-4 border-b-2 border-blue-700 pb-5">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-[-0.04em]">WonFlow Radiology Report</h1>
        ...
        <section className="mt-6 grid grid-cols-2 gap-4 rounded-xl border border-slate-200 p-5 text-sm lg:grid-cols-4">
          ...
          <div><div className="text-[10px] font-black uppercase text-slate-400">Study</div><div className="mt-1 font-bold">{order.name}</div></div>
          <div><div className="text-[10px] font-black uppercase text-slate-400">Body site</div><div className="mt-1 font-bold">{order.specimenOrBodySite ?? "—"}</div></div>
        </section>
        {released.map((result) => (
        ...
        <p className="mt-8 text-center text-[10px] leading-4 text-slate-500">Findings must be correlated with clinical history.</p>
      </article>
    </div>
  );
}
