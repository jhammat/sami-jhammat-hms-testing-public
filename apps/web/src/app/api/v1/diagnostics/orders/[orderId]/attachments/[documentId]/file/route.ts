...
import { readStaffDiagnosticAttachmentBytes } from "@/server/diagnostics/diagnostic-attachment-service";
...
export function GET(_request: Request, context: { params: Promise<{ orderId: string; documentId: string }> }) {
  return handleApiRoute(async () => {
    ...
    const { document, bytes } = await readStaffDiagnosticAttachmentBytes(await requireRequestContext(), orderId, documentId);
