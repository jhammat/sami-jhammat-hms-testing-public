...
export function GET(request: Request) { return handleApiRoute(async () => { const context = await requireRequestContext(); const url = new URL(request.url); return NextResponse.json({ catalog: await getPatientBookingCatalog(context, url.searchParams.get("date") ?? new Date().toISOString().slice(0, 10)), appointments: await listMyAppointments(context) }); }); }
