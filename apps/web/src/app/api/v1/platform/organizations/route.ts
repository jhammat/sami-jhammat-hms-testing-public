...
export async function GET(request: Request) { try { const scope = new URL(request.url).searchParams.get("scope") === "deleted" ? "deleted" : "active"; return NextResponse.json({ tenants: await service.listTenants(await requirePlatformContext(), scope) }); } catch (e) { return safeApiError(e); } }
